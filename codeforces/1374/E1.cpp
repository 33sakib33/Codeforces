...
int main()
{
    ...
    if(cy<k || cz<k) {cout<<-1<<endl;
        return 0;}
    sort(a.begin(),a.end());
    ll sum=0;
    ll ca=0,cb=0;
    ll d=0;
    //rep(i,0,n,1)cout<<a[i].ff<<" ";
    for(int i=0;i<n;i++){
        if(a[i].ss.ff==1 && a[i].ss.ss==1){
            ca++;
            cb++;
            sum=sum+a[i].ff;
            if(ca>k){
        ...
        else if(a[i].ss.ff==1 && ca<k){
              ca++;
              aq.push(a[i].ff);
              sum=sum+a[i].ff;
        }
        else if(a[i].ss.ss==1 && cb<k){
              cb++;
              bq.push(a[i].ff);
              sum=sum+a[i].ff;
        }
    ...
    for(int i=d+1;i<n;i++){
        if(a[i].ss.ff==1 && a[i].ss.ss==1){
            ...
            if(aq.top()+bq.top()>a[i].ff){
                sum=sum-aq.top()-bq.top()+a[i].ff;
                aq.pop();
                bq.pop();
            }
